...
export const Weather = () => {
  const weather = useSelector(({weather}) => weather)
    const displayIcon = () => {
        ...
        switch (number) { // les num d apres site openweather (how to get icon url)
            case '01':
            ...
            default:
                return <img src={`https://openweathermap.org/img/wn/${weather.weather.icon}@2x.png`} alt=""/> //icon weather
        }
    }
  return (
    <>
            <Card className={styles.container}>
                {weather.isLoaded ? //si weather est chargé affiche moi 
                    <Card.Body>
                        <Card.Title>
                            {weather.name} , {weather.sys.country} <PositionSvg color={'rgba(255,255,255,0.7)'}/>
                            <div className={styles.date}>
                                <div>
                                    <Moment format={'llll'}/></div>{/* npm i react-moment */}
